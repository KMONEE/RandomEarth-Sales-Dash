...
dust_master = pd.concat([total_df, tx_count_df, average_df, min_df, max_df, median_df, cum_sum], axis = 1).reset_index().sort_values(by=['BLOCK_TIMESTAMP', 'FACTION'], ascending=False)
...
def app():
    ...
    ust_pie_df = dust_master[['FACTION', 'TOTAL_UST']].groupby(['FACTION']).sum()
    ust_pie = px.pie(
    ...
    counts_pie_df = dust_master[['FACTION', 'TRANSACTION_COUNT']].groupby(['FACTION']).sum()
    counts_pie = px.pie(
